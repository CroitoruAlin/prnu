...
def main():
    ...
    for device in tqdm(test_devices):
        ...
        query_images_test = [os.path.join(config["data_path"], path) for path in list(view_ds_query['image_path'])]
        query_images_paths.extend(query_images_test)
        query_devices.extend([device]*len(query_images_test))
